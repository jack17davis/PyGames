...
from Jack_Davis4 import getScore as JackGetScore
...
def playGame(indexPlayer1, indexPlayer2):
    ...
    while state.winner < 0:
        ...
        if isHuman[state.playerToMove]:                  # Human player
            ...
            if duration.seconds + duration.microseconds * 1e-6 >= timeLimit + timeTolerance:
                ...
                if not (move in moveList):
                    ...
                    print(str(move) + " not a valid move")
                    move = moveList[0]
        ...
        displayState(state, playerNames, None)
        if debugging:
            print("Calculated score of new state as:" + str(JackGetScore(state)))
...
def computerTournament(playerIndexList):
    ...
    print("Simulating " + str(len(gameList)) + " games")
...
playerModuleList = ['Jack_Davis_Master','Jack_Davis6', 'Jack_Davis5','Jack_Davis4_branch2','Jack_Davis4','Jack_Davis3_1','Jack_Davis3','Jack_Davis2_2','Alan_Turing']
...
#for i in range(0,8):
#    computerTournament([i,8])

#computerTournament([5, 8]) # don't do more than 4 for sanity reasons
                                    # 4 is okay if you have to pee
